...
//定义$弹框扩展(手机端的精简版/resources/libs/layer/mobile/layer.js)，依赖于layer的弹框组件定制 http://layer.layui.com/mobile/
(function($){
	$.extend({
		...
		alert:function(content,title,endfunc){
			var index = layer.open({
				title: title == "no_title" ? "" : [ //或直接title:'标题'
					title ? title : "提示",
					"background-color:#3c8dbc; color:white;margin:0" //标题样式
				],
				btn: ["确定"],
				content:content,
...
$(function(){
	$("input").each(function(){
		var self=$(this);
		var checkboxClass=self.data("checkbox");
		var radioClass=self.data("radio");
		if(checkboxClass||radioClass){
			self.iCheck({
